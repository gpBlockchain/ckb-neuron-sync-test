...
export class SqliteDataComparator {
    ...
    constructor(private database1Path: string, private database2Path: string) {
    ...
    private formatTableMarkdown(rows: any[]): string {
        ...
        const formattedRows = rows.map(row =>
            `| ${keys.map(key => (row[key] !== null && row[key] !== undefined ? row[key] : 'NULL')).join(' | ')} |`
        );
    ...
    private async compareTableData(path: string, tableName: string, excludedFields: string[]) {
        return new Promise<void>((resolve, reject) => {
            this.db1.all(`SELECT *
                          FROM [${tableName}]`, (err, rowsDb1: RowData[]) => {
                if (err) {
                    reject(err);
                    return;
                }
                ...
                this.db2.all(`SELECT *
                              FROM [${tableName}]`, (err, rowsDb2: RowData[]) => {
                    if (err) {
                        reject(err);
                        return;
                    }

                    const uniqueToDb1 = rowsDb1.filter(row1 =>
                        !rowsDb2.some(row2 =>
                            Object.keys(row1)
                    ...
                    const uniqueToDb2 = rowsDb2.filter(row2 =>
                        !rowsDb1.some(row1 =>
                            Object.keys(row2)
                                .filter(key => !excludedFields.includes(key))
                                .every(key => row1[key] === row2[key])
