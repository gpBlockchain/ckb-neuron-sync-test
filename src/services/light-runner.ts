...
import {cpSync, rmdirSync, mkdirSync} from "node:fs";
...
export const startCkbLightNodeWithConfig = async (option: {
    binPath: string,
    dataPath: string,
    decPath: string
}) => {
    ...
    ckbLight = spawn("./"+ckbLightBinary(option.binPath).split("/")[ckbLightBinary(option.binPath).split("/").length-1], options, {
    ...
    ckbLight.stdout && ckbLight.stdout.on('data', data => {
        log.write(data)
    })
}
